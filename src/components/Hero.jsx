...
function VideoControls({ videoRef }) {
  ...
  return (
    <>
      ...
      {/* Top-right buttons: mute + fullscreen */}
      <div className="absolute top-2 right-2 flex items-center gap-1.5">
        <button
          onClick={toggleMute}
          className="w-7 h-7 rounded-full bg-black/30 flex items-center justify-center text-white/80"
      ...
      <div
        className="absolute bottom-0 left-0 right-0 h-4 cursor-pointer flex items-end"
        onClick={handleSeek}
        onTouchMove={handleSeek}
      >
        <div className="relative w-full h-1">
          <div className="absolute inset-0 bg-white/20 rounded-full" />
          <div className="absolute top-0 bottom-0 left-0 bg-gold/80 rounded-full transition-[width] duration-100" style={{ width: `${progress * 100}%` }} />
          <div className="absolute top-1/2 -translate-y-1/2 w-3 h-3 rounded-full bg-white shadow-md transition-[left] duration-100" style={{ left: `calc(${progress * 100}% - 6px)` }} />
        </div>
      </div>
    </>
  )
